...
First things first, let's create a new empty directory named `organise_files` in your home directory.

```bash
mkdir ~/organise_files
```
...
Once inside, create a new empty script file named `organise_files.sh`.

```bash
touch organise_files.sh
```

We will also make it executable using:

```bash
chmod u+x organise_files.sh
```
...
- Copy the `data` folder to the `organise_files` directory:
  
  ```bash
  cp -r ~/UnixShellProgramming-main/data .
  ```

  This will copy the `data` folder to the current directory, which should be the `organise_files` directory.
- Verify that the `data` folder has been copied:
  ...
  This will list the contents of the `organise_files` directory, and you should see the `data` folder.
...
```bash
Usage: ./organise_files.sh <directory>
```
...
## Step 3: organise the files based on their type
...
This `case` statement works by matching the file extension using the `*.extension` pattern. For example, `*.jpg` matches all files with the `.jpg` extension (`*` in this context is a wildcard that means "match anything"). The `|` character is used to separate multiple patterns that should be matched. The `*)` pattern is a wildcard that matches anything that did not match the previous patterns.
...
First, let's create the directories for each file type. Modify the `organise_files` function to include the following code:
...
The `mkdir -p` command is used to create the directories. The `-p` option ensures that the command does not fail if the directories already exist as well as creating any necessary parent directories. Now, we just need to move the files to the corresponding directories. Modify the `organise_files` function to include the following code:
...
We also need to change the loop to pass the full path to the file:
...
echo -e "\nOrganising files in directory: $1\n"
...
Now that we have covered the basic requirements, let us move on to the advanced features. We will now print statistics about the files moved. For example, let us measure the file size of each file moved per category. Modify the `organise_files` function to include the following code:
...
The `source` command is used to load the configuration script. This will make the variables defined in the `config.sh` script available in the main script. It effectively copies the content of the `config.sh` script into the main script. We can now use the `categories` and `file_extensions` arrays in the main script. Let us modify the `organise_files` function to use the configuration:
